...
begin
  ...
  values (new_group_id, auth.uid(), coalesce(nullif(trim(member_name), ''), 'Spelare'), 'owner', extensions.crypt(member_pin, extensions.gen_salt('bf')));
...
begin
  ...
  if existing_user_id is not null then
    if existing_pin_hash is not null and existing_pin_hash <> extensions.crypt(member_pin, existing_pin_hash) then
    ...
    if existing_user_id <> auth.uid() then
      ...
      set user_id = auth.uid(), pin_hash = extensions.crypt(member_pin, extensions.gen_salt('bf'))
      where group_id = target_group_id and user_id = existing_user_id;
    else
      update public.group_members
      set display_name = clean_name, active = true, pin_hash = extensions.crypt(member_pin, extensions.gen_salt('bf'))
      where group_id = target_group_id and user_id = auth.uid();
    end if;
  else
    insert into public.group_members (group_id, user_id, display_name, pin_hash)
    values (target_group_id, auth.uid(), clean_name, extensions.crypt(member_pin, extensions.gen_salt('bf')))
